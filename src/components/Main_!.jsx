...
const NextArrow = ({ onClick }) => (
  <div 
    ...
    onClick={onClick}
  >
    <span className="text-3xl font-bold bg-black/40 rounded-full px-2 py-1 hover:bg-red-600 transition">
      ❯
    </span>
  </div>
);
                    ...
                    {trendData.length <= 3 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 gap-y-4">
                            {trendData.map(trend => (
                                <div 
                                    key={trend.id}
                                    className=" rounded-md overflow-hidden h-[300px] w-[300px] transition-transform hover:scale-110 border border-red-600 shadow-lg "
                                >

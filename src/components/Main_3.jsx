...
const NextArrow = ({ onClick, disabled }) => (
  <div 
    className={`absolute right-2 sm:-right-6 md:-right-8 lg:-right-12 top-1/2 -translate-y-1/2 z-10 cursor-pointer text-white
      ${disabled ? "opacity-40 pointer-events-none" : ""}`}
    onClick={disabled ? undefined : onClick}
  >
    <span className="text-xl sm:text-2xl md:text-3xl font-bold bg-black/40 rounded-full px-2 py-1 hover:bg-red-600 transition">
      ❯
    </span>
  </div>
);
...
const PrevArrow = ({ onClick, disabled }) => (
  <div 
    className={`absolute left-2 sm:-left-4 md:-left-6 lg:-left-10 top-1/2 -translate-y-1/2 z-10 cursor-pointer text-white
      ${disabled ? "opacity-40 pointer-events-none" : ""}`}
    onClick={disabled ? undefined : onClick}
  >
    <span className="text-xl sm:text-2xl md:text-3xl font-bold bg-black/40 rounded-full px-2 py-1 hover:bg-red-600 transition">
...
const Main = () => {
  ...
  // Cantidad máxima de slides visibles dinámicamente
